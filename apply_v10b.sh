#!/usr/bin/env bash
# apply_v10b.sh — Corrige el bug del microfono que se cierra a mitad de
# oracion: sesion logica auto-reiniciable sobre el SpeechRecognizer de
# Android (que corta a los 30-60s ignorando listenFor).
set -euo pipefail
test -f pubspec.yaml || { echo "Corre desde la raíz del repo"; exit 1; }
cat > lib/services/voice_prayer_service.dart <<'EOF_V10B'
